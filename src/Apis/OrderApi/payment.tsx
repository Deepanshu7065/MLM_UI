...
// ✅ Razorpay fields hataaye, Cashfree fields rakhe
export type VerifyPaymentType = {
  payment_db_id: string;
  order_id: string;
};
...
export const PaymentApi = {
  ...
  updatePaymentStatus: async ({ id, data }: { id: string; data: any }) => {
    const res = await api.patch(`${baseUrl}/update-payment/${id}`, data, {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    });
    return res.data;
  },
  ...
  updateWithdrawStatus: async ({ id, data }: { id?: string | null; data: any }) => {
    const res = await api.patch(`${baseUrl}/update-withdraw/${id}`, data, {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    });
    return res.data;
  },
